	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussyuejuxinxi`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussyuejuxinxi`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`discussyuejuxinxi`, this.ruleForm);
						}else{
							oet = await this.$api.add(`discussyuejuxinxi`, this.ruleForm);
						}
						uni.setStorageSync('discussyuejuxinxiCleanType',true);

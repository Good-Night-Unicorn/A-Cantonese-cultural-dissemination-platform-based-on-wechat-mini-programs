			...
			<view :style='{"width":"100%","padding":"0 0 160rpx","position":"relative","background":"#F1F5F6","height":"auto"}'>
				<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"20rpx 24rpx","background":"#43B3BE","display":"flex","height":"auto"}'>
					<view :style='{"margin":"0 0","position":"relative","flex":"1","background":"none"}' class="search-form round">
						...
						<input placeholder-style="color: #000" :style='{"border":"0","padding":"12rpx 20rpx 12rpx 80rpx","color":"#000","borderRadius":"10rpx 0 0 10rpx","background":"rgba(255,255,255,0.5)","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.yingshimingcheng" type="text" placeholder="影视名称" ></input>
					</view>
				...
				<view :style='{"width":"100%","background":"none","height":"auto"}'>
					<scroll-view scroll-x="true" class="category-one" :style='{"padding":"20rpx","margin":"0 auto","whiteSpace":"nowrap","background":"#fff","display":"flex","width":"calc(100% - 40rpx)","height":"auto"}'>
						<view :class='categoryName === item.yingshifenlei ? "active" : ""' class="tab" v-for="(item,index) in categoryList" :key="index" @tap="categoryClick(item.yingshifenlei)">{{item.yingshifenlei}}</view>
					</scroll-view>
					<!-- 样式2 -->
					<view class="list" :style='{"width":"calc(100% - 40rpx)","padding":"20rpx 20rpx 0","margin":"0 auto","borderRadius":"0 0 20rpx 20rpx","background":"#fff","height":"auto"}'>
						<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","margin":"0 0 20rpx","borderColor":"#F1F5F6","flexWrap":"wrap","borderWidth":"0 0 2rpx","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' v-for="(product,index) in list" :key="index">
							...
							<view class="listmr" :style='{"width":"calc(100% - 200rpx)","padding":"0","margin":"0","flexWrap":"wrap","display":"flex","height":"auto"}'>
								<view class="col3 f30 elip mb15" :style='{"width":"70%","padding":"0 20rpx","margin":"0","lineHeight":"48rpx","fontSize":"32rpx","color":"#000"}'>{{product.yingshimingcheng}}</view>
							...
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('yueyuyingshi','修改')) || (!userid && isAuthFront('yueyuyingshi','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('yueyuyingshi','删除')) || (!userid && isAuthFront('yueyuyingshi','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('yueyuyingshi','修改')) || (!userid && isAuthFront('yueyuyingshi','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"50rpx","color":"#43B3BE","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"50rpx","color":"#43B3BE","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('yueyuyingshi','删除')) || (!userid && isAuthFront('yueyuyingshi','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"50rpx","color":"#959595","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"50rpx","color":"#959595","display":"inline-block"}'>删除</text>
								</view>
							</view>
				...
				<view class="screenBox" :class="screenBoxShow?'screenBoxActive':''">
					<view :style='{"width":"100%","padding":"20rpx 0 20rpx 0"}'>
						<view :style='{"width":"100%","padding":"0 0 20rpx 20rpx","fontSize":"24rpx"}'>影视分类</view>
						<view :style='{"width":"100%","flexWrap":"wrap","justifyContent":"space-around","display":"flex"}'>
							<view @click="yingshifenleiChange(item)" v-for="(item,index) in yingshifenleiOptions" :key="index" class="screenTab" :class="searchForm.yingshifenlei==item?'screenTabActive':''">{{item}}</view>
						</view>
					</view>
					<view :style='{"width":"100%","padding":"20rpx 0 20rpx 0","alignItems":"center","flexWrap":"wrap","justifyContent":"space-around","display":"flex"}'>
						<view :style='{"width":"100%","padding":"0 0 20rpx 20rpx","fontSize":"24rpx"}'>导演</view>
						<input :style='{"border":"2rpx solid #D9DADC","width":"90%","padding":"0 20rpx 0 20rpx","borderRadius":"40rpx","background":"#F8F8F8","height":"80rpx"}' placeholder="请输入导演" v-model="searchForm.daoyan">
					</view>
					<view :style='{"width":"100%","padding":"20rpx 0 20rpx 0","alignItems":"center","flexWrap":"wrap","justifyContent":"space-around","display":"flex"}'>
						<view :style='{"width":"100%","padding":"0 0 20rpx 20rpx","fontSize":"24rpx"}'>参与演员</view>
						<input :style='{"border":"2rpx solid #D9DADC","width":"90%","padding":"0 20rpx 0 20rpx","borderRadius":"40rpx","background":"#F8F8F8","height":"80rpx"}' placeholder="请输入参与演员" v-model="searchForm.canyuyanyuan">
					</view>
...
<script>
	export default {
		...
		async onShow() {
            ...
            let res = {};
            if(this.userid) {
                res = await this.$api.page('yingshifenlei', {page:1,limit:100});
            } else {
                res = await this.$api.list('yingshifenlei', {page:1,limit:100});
            }
			res.data.list.splice(0,0,{id:0,yingshifenlei:'全部'})
		...
		async onLoad(options) {
			...
			let res = {};
			//yingshifenlei筛选
			res = await this.$api.option(`yingshifenlei`,`yingshifenlei`,{});
			this.yingshifenleiOptions = res.data;
		},
		components: {
		},
		methods: {
			yingshifenleiChange(e){
				this.searchForm.yingshifenlei = e
				this.$forceUpdate()
			},
			screenReset(){
				this.searchForm = {}
			...
			async upCallback(mescroll) {
				...
				if(this.categoryName!='全部'){
					params.yingshifenlei = '%' + this.categoryName + '%'
				}
				if(this.searchForm.yingshimingcheng){
					params['yingshimingcheng'] = '%' + this.searchForm.yingshimingcheng + '%'
				}
				if(this.searchForm.yingshifenlei){
					params['yingshifenlei'] = '%' + this.searchForm.yingshifenlei + '%'
				}
				if(this.searchForm.daoyan){
					params['daoyan'] = '%' + this.searchForm.daoyan + '%'
				}
				if(this.searchForm.canyuyanyuan){
					params['canyuyanyuan'] = '%' + this.searchForm.canyuyanyuan + '%'
				}
				let user = uni.getStorageSync("appUserid")?JSON.parse(uni.getStorageSync('userSession')):{}
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`yueyuyingshi`, params);
                } else {
                    res = await this.$api.list(`yueyuyingshi`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('yueyuyingshi', JSON.stringify([id]));
			...
			async search(){
				...
				if(this.searchForm.yingshimingcheng){
					searchForm['yingshimingcheng'] = '%' + this.searchForm.yingshimingcheng + '%'
				}
				if(this.categoryName!='全部'){
					searchForm.yingshifenlei = '%' + this.categoryName + '%'
				}
				if(this.searchForm.yingshifenlei){
					searchForm['yingshifenlei'] = '%' + this.searchForm.yingshifenlei + '%'
				}
				if(this.searchForm.daoyan){
					searchForm['daoyan'] = '%' + this.searchForm.daoyan + '%'
				}
				if(this.searchForm.canyuyanyuan){
					searchForm['canyuyanyuan'] = '%' + this.searchForm.canyuyanyuan + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`yueyuyingshi`, searchForm);
                } else {
                    res = await this.$api.list(`yueyuyingshi`, searchForm);
                }
				// 如果是第一页数据置空

			...
			<view :style='{"width":"100%","padding":"0 0 160rpx","position":"relative","background":"#F1F5F6","height":"auto"}'>
				<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"20rpx 24rpx","background":"#43B3BE","display":"flex","height":"auto"}'>
					<view :style='{"margin":"0 0","position":"relative","flex":"1","background":"none"}' class="search-form round">
						...
						<input placeholder-style="color: #000" :style='{"border":"0","padding":"12rpx 20rpx 12rpx 80rpx","color":"#000","borderRadius":"10rpx 0 0 10rpx","background":"rgba(255,255,255,0.5)","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.yingshifenlei" type="text" placeholder="影视分类" ></input>
					</view>
					...
					<view class="list" :style='{"width":"calc(100% - 40rpx)","padding":"20rpx 20rpx 0","margin":"0 auto","borderRadius":"0 0 20rpx 20rpx","background":"#fff","height":"auto"}'>
						<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","margin":"0 0 20rpx","borderColor":"#F1F5F6","flexWrap":"wrap","borderWidth":"0 0 2rpx","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' v-for="(product,index) in list" :key="index">
							...
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('yingshifenlei','修改')) || (!userid && isAuthFront('yingshifenlei','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('yingshifenlei','删除')) || (!userid && isAuthFront('yingshifenlei','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('yingshifenlei','修改')) || (!userid && isAuthFront('yingshifenlei','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"50rpx","color":"#43B3BE","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"50rpx","color":"#43B3BE","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('yingshifenlei','删除')) || (!userid && isAuthFront('yingshifenlei','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"50rpx","color":"#959595","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"50rpx","color":"#959595","display":"inline-block"}'>删除</text>
								</view>
							</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`yingshifenlei`, params);
                } else {
                    res = await this.$api.list(`yingshifenlei`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('yingshifenlei', JSON.stringify([id]));
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`yingshifenlei`, searchForm);
                } else {
                    res = await this.$api.list(`yingshifenlei`, searchForm);
                }
				// 如果是第一页数据置空

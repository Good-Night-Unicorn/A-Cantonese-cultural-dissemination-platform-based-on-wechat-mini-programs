	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussyueyuwenhua`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussyueyuwenhua`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`discussyueyuwenhua`, this.ruleForm);
						}else{
							oet = await this.$api.add(`discussyueyuwenhua`, this.ruleForm);
						}
						uni.setStorageSync('discussyueyuwenhuaCleanType',true);

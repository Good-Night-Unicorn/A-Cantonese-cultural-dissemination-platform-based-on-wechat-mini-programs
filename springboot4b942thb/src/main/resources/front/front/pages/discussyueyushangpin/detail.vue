		...
		<view class="container" :style='{"padding":"20rpx","flexWrap":"wrap","background":"#F1F5F6","display":"flex","width":"100%","position":"relative","height":"auto"}'>
			<view :style='{"width":"100%","padding":"20rpx 0","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}' class="detail-content">
				...
				<view class="detail-list-item" :style='{"width":"100%","margin":"0 0 10rpx","alignItems":"center","flexWrap":"wrap","display":"flex","height":"auto"}'>
					...
					<view class="text" :style='{"padding":"0px","margin":"0px","lineHeight":"80rpx","fontSize":"28rpx","color":"#929292","flex":"1"}' >{{detail.refid}}</view>
				</view>
				<view class="detail-list-item" :style='{"width":"100%","margin":"0 0 10rpx","alignItems":"center","flexWrap":"wrap","display":"flex","height":"auto"}'>
					...
					<view class="text" :style='{"padding":"0px","margin":"0px","lineHeight":"80rpx","fontSize":"28rpx","color":"#929292","flex":"1"}' >{{detail.istop}}</view>
				</view>
				...
				<view class="detail-list-item" :style='{"width":"100%","margin":"0 0 10rpx","alignItems":"center","flexWrap":"wrap","display":"flex","height":"auto"}'>
					...
					<view class="text" :style='{"padding":"0px","margin":"0px","lineHeight":"80rpx","fontSize":"28rpx","color":"#929292","flex":"1"}'>{{detail.content}}</view>
				</view>
				<view class="detail-list-item" :style='{"width":"100%","margin":"0 0 10rpx","alignItems":"center","flexWrap":"wrap","display":"flex","height":"auto"}'>
					...
					<view class="text" :style='{"padding":"0px","margin":"0px","lineHeight":"80rpx","fontSize":"28rpx","color":"#929292","flex":"1"}'>{{detail.reply}}</view>
				</view>
				<view class="detail-list-item" :style='{"width":"100%","margin":"0 0 10rpx","alignItems":"center","flexWrap":"wrap","display":"flex","height":"auto"}'>
					...
					<view class="text" :style='{"padding":"0px","margin":"0px","lineHeight":"80rpx","fontSize":"28rpx","color":"#929292","flex":"1"}'>{{detail.tuserids}}</view>
				</view>
				<view class="detail-list-item" :style='{"width":"100%","margin":"0 0 10rpx","alignItems":"center","flexWrap":"wrap","display":"flex","height":"auto"}'>
					...
					<view class="text" :style='{"padding":"0px","margin":"0px","lineHeight":"80rpx","fontSize":"28rpx","color":"#929292","flex":"1"}'>{{detail.cuserids}}</view>
				</view>
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"width":"100%","padding":"10rpx","flexWrap":"wrap","background":"#fff","display":"flex","height":"auto"}'>
					...
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0 10rpx 10rpx 0","color":"#000","background":"#F1F5F6","width":"auto","fontSize":"28rpx","lineHeight":"60rpx","height":"60rpx"}' v-if="userid&&isAuth('discussyueyushangpin','私聊')" @tap="chatClick">联系TA</button>
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0 10rpx 10rpx 0","color":"#000","background":"#F1F5F6","width":"auto","fontSize":"28rpx","lineHeight":"60rpx","height":"60rpx"}' v-if="!userid&&isAuthFront('discussyueyushangpin','私聊')" @tap="chatClick">联系TA</button>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussyueyushangpinCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussyueyushangpinCleanType')
				this.mescroll.num = 1
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('discussyueyushangpin', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','discussyueyushangpin');
			...
			async onAcrossTap(tableName,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				uni.setStorageSync('crossTable','discussyueyushangpin');
			...
			async init(type=1){
				...
				let res = await this.$api.info('discussyueyushangpin', this.id);
				let reg=new RegExp('http://localhost:8080/springboot4b942thb/upload','g')//g代表全部
				this.detail = res.data;
				...
				if(type==2){
					...
					await this.$api.update('discussyueyushangpin',this.detail)
				}
			},
			...
			async upCallback(mescroll) {
				if(uni.getStorageSync("appUserid")){
					let res = await this.$api.list('discussdiscussyueyushangpin', {
						page: mescroll.num,
						limit: 10,
			...
			async comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					await this.$api.update('discussdiscussyueyushangpin',row)
					this.$utils.msg('点赞成功');
				}else {
					row.thumbsupnum--
					let arr = String(row.tuserids).split(',')
			...
			async onCommentTap() {
				...
				let res = {}
				this.$utils.jump(`../discussdiscussyueyushangpin/add-or-update?refid=${this.id}` )
			},
			delClick(id){
				let that = this
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscussyueyushangpin',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discussyueyushangpin',that.detail)
							setTimeout(()=>{

	...
	<view :style='{"minHeight":"100%","width":"100%","padding":"0","position":"relative","background":"#fff","height":"auto"}'>
		<form :style='{"width":"100%","padding":"24rpx","background":"#fff","display":"block","height":"auto"}' class="app-update-pv">
			<view :style='{"width":"100%","padding":"12rpx 0","margin":"0 0 20rpx 0","alignItems":"flex-start","display":"flex","height":"auto"}' class="">
				...
				<input :style='{"border":"2rpx solid #D9DADC","padding":"0px 20rpx","margin":"0px","color":"rgb(0, 0, 0)","borderRadius":"8rpx","flex":"1","background":"#F8F8F8","fontSize":"28rpx","height":"80rpx"}' :disabled="ro.yuejufenlei" v-model="ruleForm.yuejufenlei" placeholder="粤剧分类"  type="text"></input>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`yuejufenlei`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`yuejufenlei`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`yuejufenlei`, this.ruleForm);
						}else{
							oet = await this.$api.add(`yuejufenlei`, this.ruleForm);
						}

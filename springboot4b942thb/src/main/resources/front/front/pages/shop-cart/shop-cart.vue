					...
					<view class="glance-shop-cart-scrollx-items-item">
						...
						<view class="glance-shop-cart-items-item-des">
							...
							<view class="glance-shop-cart-items-item-pq">
								<view class="sigle-line-text" style="font-size: 15px;text-align: left;width: 50%;">￥{{priceChange(item)}}</view>
								<!-- 数量操作 -->
							...
							await uni.showModal({
								...
								success: async function(res) {
									if(res.confirm){
										...
										uni.removeStorageSync(`cart${that.cart[i].tablename}${that.cart[i].goodid}${that.cart[i].userid}`)
										that.init();
									}
								}
							})

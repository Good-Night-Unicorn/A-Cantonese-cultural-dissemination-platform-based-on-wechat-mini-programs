	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussyinlexinxi`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussyinlexinxi`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`discussyinlexinxi`, this.ruleForm);
						}else{
							oet = await this.$api.add(`discussyinlexinxi`, this.ruleForm);
						}
						uni.setStorageSync('discussyinlexinxiCleanType',true);

		...
		<view class="container" :style='{"padding":"20rpx","flexWrap":"wrap","background":"#F1F5F6","display":"flex","width":"100%","position":"relative","height":"auto"}'>
			<view :style='{"width":"100%","padding":"20rpx 0","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}' class="detail-content">
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"width":"100%","padding":"10rpx","flexWrap":"wrap","background":"#fff","display":"flex","height":"auto"}'>
					...
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0 10rpx 10rpx 0","color":"#000","background":"#F1F5F6","width":"auto","fontSize":"28rpx","lineHeight":"60rpx","height":"60rpx"}' v-if="userid&&isAuth('discussyinlexinxi','私聊')" @tap="chatClick">联系TA</button>
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0 10rpx 10rpx 0","color":"#000","background":"#F1F5F6","width":"auto","fontSize":"28rpx","lineHeight":"60rpx","height":"60rpx"}' v-if="!userid&&isAuthFront('discussyinlexinxi','私聊')" @tap="chatClick">联系TA</button>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussyinlexinxiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussyinlexinxiCleanType')
				this.mescroll.num = 1
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('discussyinlexinxi', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','discussyinlexinxi');
			...
			async onAcrossTap(tableName,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				uni.setStorageSync('crossTable','discussyinlexinxi');
			...
			async init(type=1){
				...
				let res = await this.$api.info('discussyinlexinxi', this.id);
				let reg=new RegExp('http://localhost:8080/springboot4b942thb/upload','g')//g代表全部
				this.detail = res.data;
				...
				if(type==2){
					...
					await this.$api.update('discussyinlexinxi',this.detail)
				}
			},
			...
			async upCallback(mescroll) {
				if(uni.getStorageSync("appUserid")){
					let res = await this.$api.list('discussdiscussyinlexinxi', {
						page: mescroll.num,
						limit: 10,
			...
			async comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					await this.$api.update('discussdiscussyinlexinxi',row)
					this.$utils.msg('点赞成功');
				}else {
					row.thumbsupnum--
					let arr = String(row.tuserids).split(',')
			...
			async onCommentTap() {
				...
				let res = {}
				this.$utils.jump(`../discussdiscussyinlexinxi/add-or-update?refid=${this.id}` )
			},
			delClick(id){
				let that = this
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscussyinlexinxi',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discussyinlexinxi',that.detail)
							setTimeout(()=>{

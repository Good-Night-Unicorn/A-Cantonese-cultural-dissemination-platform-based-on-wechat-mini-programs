			...
			<view :style='{"width":"100%","padding":"0 0 160rpx","position":"relative","background":"#F1F5F6","height":"auto"}'>
				<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"20rpx 24rpx","background":"#43B3BE","display":"flex","height":"auto"}'>
					<view :style='{"margin":"0 0","position":"relative","flex":"1","background":"none"}' class="search-form round">
						...
						<input placeholder-style="color: #000" :style='{"border":"0","padding":"12rpx 20rpx 12rpx 80rpx","color":"#000","borderRadius":"10rpx 0 0 10rpx","background":"rgba(255,255,255,0.5)","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.yuejumingcheng" type="text" placeholder="粤剧名称" ></input>
					</view>
				...
				<view :style='{"width":"100%","background":"none","height":"auto"}'>
					<scroll-view scroll-x="true" class="category-one" :style='{"padding":"20rpx","margin":"0 auto","whiteSpace":"nowrap","background":"#fff","display":"flex","width":"calc(100% - 40rpx)","height":"auto"}'>
						<view :class='categoryName === item.yuejufenlei ? "active" : ""' class="tab" v-for="(item,index) in categoryList" :key="index" @tap="categoryClick(item.yuejufenlei)">{{item.yuejufenlei}}</view>
					</scroll-view>
					<!-- 样式2 -->
					<view class="list" :style='{"width":"calc(100% - 40rpx)","padding":"20rpx 20rpx 0","margin":"0 auto","borderRadius":"0 0 20rpx 20rpx","background":"#fff","height":"auto"}'>
						<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","margin":"0 0 20rpx","borderColor":"#F1F5F6","flexWrap":"wrap","borderWidth":"0 0 2rpx","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' v-for="(product,index) in list" :key="index">
							...
							<view class="listmr" :style='{"width":"calc(100% - 200rpx)","padding":"0","margin":"0","flexWrap":"wrap","display":"flex","height":"auto"}'>
								<view class="col3 f30 elip mb15" :style='{"width":"70%","padding":"0 20rpx","margin":"0","lineHeight":"48rpx","fontSize":"32rpx","color":"#000"}'>{{product.yuejumingcheng}}</view>
							...
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('yuejuxinxi','修改')) || (!userid && isAuthFront('yuejuxinxi','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('yuejuxinxi','删除')) || (!userid && isAuthFront('yuejuxinxi','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('yuejuxinxi','修改')) || (!userid && isAuthFront('yuejuxinxi','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"50rpx","color":"#43B3BE","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"50rpx","color":"#43B3BE","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('yuejuxinxi','删除')) || (!userid && isAuthFront('yuejuxinxi','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"50rpx","color":"#959595","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"50rpx","color":"#959595","display":"inline-block"}'>删除</text>
								</view>
							</view>
				...
				<view class="screenBox" :class="screenBoxShow?'screenBoxActive':''">
					<view :style='{"width":"100%","padding":"20rpx 0 20rpx 0"}'>
						<view :style='{"width":"100%","padding":"0 0 20rpx 20rpx","fontSize":"24rpx"}'>粤剧分类</view>
						<view :style='{"width":"100%","flexWrap":"wrap","justifyContent":"space-around","display":"flex"}'>
							<view @click="yuejufenleiChange(item)" v-for="(item,index) in yuejufenleiOptions" :key="index" class="screenTab" :class="searchForm.yuejufenlei==item?'screenTabActive':''">{{item}}</view>
						</view>
					</view>
...
<script>
	export default {
		...
		async onShow() {
            ...
            let res = {};
            if(this.userid) {
                res = await this.$api.page('yuejufenlei', {page:1,limit:100});
            } else {
                res = await this.$api.list('yuejufenlei', {page:1,limit:100});
            }
			res.data.list.splice(0,0,{id:0,yuejufenlei:'全部'})
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.categoryName!='全部'){
					params.yuejufenlei = '%' + this.categoryName + '%'
				}
				if(this.searchForm.yuejumingcheng){
					params['yuejumingcheng'] = '%' + this.searchForm.yuejumingcheng + '%'
				}
				if(this.searchForm.yuejufenlei){
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`yuejuxinxi`, params);
                } else {
                    res = await this.$api.list(`yuejuxinxi`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('yuejuxinxi', JSON.stringify([id]));
			...
			async search(){
				...
				if(this.searchForm.yuejumingcheng){
					searchForm['yuejumingcheng'] = '%' + this.searchForm.yuejumingcheng + '%'
				}
				if(this.categoryName!='全部'){
					searchForm.yuejufenlei = '%' + this.categoryName + '%'
				}
				if(this.searchForm.yuejufenlei){
					searchForm['yuejufenlei'] = '%' + this.searchForm.yuejufenlei + '%'
				}
				if(this.searchForm.daoyan){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`yuejuxinxi`, searchForm);
                } else {
                    res = await this.$api.list(`yuejuxinxi`, searchForm);
                }
				// 如果是第一页数据置空

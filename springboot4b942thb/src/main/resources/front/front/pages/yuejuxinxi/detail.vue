		...
		<view class="container" :style='{"padding":"20rpx","flexWrap":"wrap","background":"#F1F5F6","display":"flex","width":"100%","position":"relative","height":"auto"}'>
			...
			<view :style='{"width":"100%","padding":"20rpx 0","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}' class="detail-content">
				...
				<view :style='{"width":"100%","margin":"0 0 10rpx","alignItems":"center","flexWrap":"wrap","display":"flex","height":"auto"}' class="detail-list-item title">
					<view :style='{"width":"160rpx","padding":"0 20rpx 0 0","lineHeight":"80rpx","fontSize":"28rpx","color":"#929292","textAlign":"right"}' class="lable">粤剧名称：</view>
					<view :style='{"padding":"0px","margin":"0px","lineHeight":"80rpx","fontSize":"28rpx","color":"#929292","flex":"1"}' class="text" >{{detail.yuejumingcheng}}</view>
				</view>

				<view class="detail-list-item" :style='{"width":"100%","margin":"0 0 10rpx","alignItems":"center","flexWrap":"wrap","display":"flex","height":"auto"}'>
					<view class="lable" :style='{"width":"160rpx","padding":"0 20rpx 0 0","lineHeight":"80rpx","fontSize":"28rpx","color":"#929292","textAlign":"right"}'>粤剧分类：</view>
					<view class="text" :style='{"padding":"0px","margin":"0px","lineHeight":"80rpx","fontSize":"28rpx","color":"#929292","flex":"1"}' >{{detail.yuejufenlei}}</view>
				</view>
				<view class="detail-list-item" :style='{"width":"100%","margin":"0 0 10rpx","alignItems":"center","flexWrap":"wrap","display":"flex","height":"auto"}'>
				...
				<view class="detail-list-item rich" :style='{"width":"100%","margin":"0 0 10rpx 0","display":"flex","height":"auto"}'>
					<view class="lable" :style='{"padding":"0 20rpx 0 0","color":"#333","textAlign":"right","display":"none","width":"160rpx","lineHeight":"80rpx","fontSize":"28rpx"}'>粤剧介绍：</view>
					<view class="rich-text" :style='{"color":"#384638","padding":"24rpx 24rpx 24rpx 0","margin":"0"}'>
						<rich-text :nodes="detail.yuejujieshao"></rich-text>
					</view>
				</view>
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"width":"100%","padding":"10rpx","flexWrap":"wrap","background":"#fff","display":"flex","height":"auto"}'>
					...
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0 10rpx 10rpx 0","color":"#000","background":"#F1F5F6","width":"auto","fontSize":"28rpx","lineHeight":"60rpx","height":"60rpx"}' v-if="userid&&isAuth('yuejuxinxi','私聊')" @tap="chatClick">联系TA</button>
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0 10rpx 10rpx 0","color":"#000","background":"#F1F5F6","width":"auto","fontSize":"28rpx","lineHeight":"60rpx","height":"60rpx"}' v-if="!userid&&isAuthFront('yuejuxinxi','私聊')" @tap="chatClick">联系TA</button>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			this.getStoreup();
			this.getThumbsup();
			let cleanType = uni.getStorageSync('discussyuejuxinxiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussyuejuxinxiCleanType')
				this.mescroll.num = 1
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('yuejuxinxi', this.id);
				let reg=new RegExp('http://localhost:8080/springboot4b942thb/upload','g')//g代表全部
				if(res.data.yuejujieshao){
					res.data.yuejujieshao = res.data.yuejujieshao.replace(reg, this.$base.url + 'upload');
				}
				this.detail = res.data;
				this.title = this.detail.yuejumingcheng
			}
		},
		destroyed: function() {
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','yuejuxinxi');
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								...
								await _this.$api.update('yuejuxinxi',_this.detail)
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.add('storeup', {
								userid: _this.user.id,
								name: _this.detail.yuejumingcheng,
			...
			async onAcrossTap(tableName,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				uni.setStorageSync('crossTable','yuejuxinxi');
			...
			async init(type=1){
				...
				let res = await this.$api.info('yuejuxinxi', this.id);
				let reg=new RegExp('http://localhost:8080/springboot4b942thb/upload','g')//g代表全部
				res.data.yuejujieshao = res.data.yuejujieshao.replace(reg, this.$base.url + 'upload');
				this.detail = res.data;
				...
				//修改富文本的图片样式
				this.detail.yuejujieshao = this.detail.yuejujieshao.replace(/img src/gi,"img style=\"width:100%;\" src");
				...
				if(type==2){
					...
					await this.$api.update('yuejuxinxi',this.detail)
				}
			},
			...
			async upCallback(mescroll) {
				if(uni.getStorageSync("appUserid")){
					let res = await this.$api.list('discussyuejuxinxi', {
						page: mescroll.num,
						limit: 10,
			...
			async comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					await this.$api.update('discussyuejuxinxi',row)
					this.$utils.msg('点赞成功');
				}else {
					row.thumbsupnum--
					let arr = String(row.tuserids).split(',')
			...
			async onCommentTap() {
				...
				let res = {}
				this.$utils.jump(`../discussyuejuxinxi/add-or-update?refid=${this.id}` )
			},
			delClick(id){
				let that = this
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussyuejuxinxi',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('yuejuxinxi',that.detail)
							setTimeout(()=>{

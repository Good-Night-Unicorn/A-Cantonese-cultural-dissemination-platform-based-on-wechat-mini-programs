	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussyueyuyingshi`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussyueyuyingshi`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`discussyueyuyingshi`, this.ruleForm);
						}else{
							oet = await this.$api.add(`discussyueyuyingshi`, this.ruleForm);
						}
						uni.setStorageSync('discussyueyuyingshiCleanType',true);

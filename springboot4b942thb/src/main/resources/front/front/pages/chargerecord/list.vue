			...
			<view :style='{"width":"100%","padding":"0 0 160rpx","position":"relative","background":"#F1F5F6","height":"auto"}'>
				<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"20rpx 24rpx","background":"#43B3BE","display":"flex","height":"auto"}'>
					<view :style='{"margin":"0 0","position":"relative","flex":"1","background":"none"}' class="search-form round">
						...
						<input placeholder-style="color: #000" :style='{"border":"0","padding":"12rpx 20rpx 12rpx 80rpx","color":"#000","borderRadius":"10rpx 0 0 10rpx","background":"rgba(255,255,255,0.5)","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.username" type="text" placeholder="用户名" ></input>
					</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`chargerecord`, params);
                } else {
                    res = await this.$api.list(`chargerecord`, params);
                }
			...
			onUpdateOrDel(row){
				...
				let that = this
				let arr = []
				if((this.userid && this.isAuth('chargerecord','修改')) || (!this.userid && this.isAuthFront('chargerecord','修改'))){
					arr.push('修改')
				}
				if((this.userid && this.isAuth('chargerecord','删除')) || (!this.userid && this.isAuthFront('chargerecord','删除'))){
					arr.push('删除')
				}
				if(!arr.length){
					return false
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('chargerecord', JSON.stringify([id]));
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`chargerecord`, searchForm);
                } else {
                    res = await this.$api.list(`chargerecord`, searchForm);
                }
				// 如果是第一页数据置空
